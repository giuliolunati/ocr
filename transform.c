...
void splitx_image(void **out1, void **out2, image *im, float x) {
  ...
  uint w1= w * x;
  uint w2= w - w1;
  image* im1= clone_image(im, w1, h);
  image* im2= clone_image(im, w2, h);
  for (z= 0; z < 5; z++) {
    p= im->chan[z];
    if (! p) continue;
    p1= im1->chan[z];
...
void splity_image(void **out1, void **out2, image *im, float y) {
  ...
  image* im1= clone_image(im, w, h1);
  image* im2= clone_image(im, w, h2);
  l1= w * h1;
  l2= w * h2;
  gray *p;
  for (z= 0; z < 5; z++) {
...
image *crop(image *im, int x1, int y1, int x2, int y2) {
  ...
  image *out= clone_image(im, w1, h1);
  out->ex= im->ex;
  out->pag= im->pag;
  for (z= 0; z < 5; z++) {
...
real detect_skew(image *im) {
  ...
  real t, s= 0;
  // create test image
  image *test= clone_image(im, w, h-1);
  gray *p1, *p2, *pt, *end;
  for (y= 0; y < h - 1; y++) {
    p1= im->chan[1] + y * w;
    end= p2= p1 + w;
